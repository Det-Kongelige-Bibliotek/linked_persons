...
module LP
  module Fetcher
    ...
    def fetch(uri_str, limit = 10)
      ...
      raise Errors::TooManyRedirects if limit == 0
      ...
      when Net::HTTPSuccess then
        ...
        if ! CONTENT_TYPES.include?(response.content_type)
          raise Errors::UnsupportedContentType, response.content_type
        end
        response

      when Net::HTTPRedirection then
        location = response['location']      
        raise Errors::CouldNotFetch, uri_str if !location
        ...
        raise Errors::CouldNotFetch, uri_str
      end
    end
